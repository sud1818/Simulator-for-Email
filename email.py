...
if "deleted_email_ids" not in st.session_state:
    st.session_state.deleted_email_ids = []
...
# --- Page 2: Inbox (Email Read) ---
    ...
    with col1:
        ...
        # Filter out any deleted emails
        visible_emails = [email for email in MOCK_INBOX if email['id'] not in st.session_state.deleted_email_ids]
        
        if not visible_emails:
            st.info("Your inbox is empty. Reset the inbox to see mock emails.")
        ...
        for email in visible_emails:
            with st.container(border=True):
                ...
                with b_col2:
                    if st.button("Delete", key=f"delete_{email['id']}", use_container_width=True):
                        st.session_state.deleted_email_ids.append(email['id'])
                        # If we delete the currently selected email, clear the view
